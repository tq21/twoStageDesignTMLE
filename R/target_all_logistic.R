...
library(nleqslv)
library(sl3)
target_all_logistic <- function(Y, A, W, W_all, Delta.W, W.stage2,
                                QAW=NULL, Q0W=NULL, Q1W=NULL, g1W=NULL,
                                pool=TRUE,
                                Z=NULL, Delta = rep(1, length(Y)), pi=NULL, piform=NULL, pi_oracle=NULL,
                                DFullReg_true = NULL,
                                Q_sl_lib = list(Lrnr_glm$new(), Lrnr_glmnet$new()),
                                g_sl_lib = list(Lrnr_glm$new(), Lrnr_glmnet$new()),
                                DFullReg_sl_lib = list(Lrnr_glm$new(), Lrnr_glmnet$new()),
                                pi.SL.library = c("SL.glm", "SL.gam", "SL.glmnet"),
                                DFullReg.library = c("SL.glm", "SL.glmnet", "SL.gam"),
  ...
  psi <- mean(Delta.W/res.twoStage$pi*(Q1W-Q0W))
  DFull <- HAW*(Y-QAW)+Q1W-Q0W-psi

  # estimate m=E(DFullNC|Delta=1,V)
  DFull_data <- cbind(res.twoStage$d.pi, DFull = DFull, Delta.W = Delta.W)
  DFull_data_pred <- DFull_data; DFull_data_pred$Delta.W <- 1
  DFull_task <- sl3_Task$new(data = DFull_data,
                             covariates = c(names(res.twoStage$d.pi), "Delta.W"),
                             outcome = "DFull", outcome_type = "gaussian")
  DFull_data_pred_task <- sl3_Task$new(data = DFull_data_pred,
                                       covariates = c(names(res.twoStage$d.pi), "Delta.W"),
                                       outcome = "DFull", outcome_type = "gaussian")
  ...
  suppressMessages(DFull_fit_sl <- DFullReg_lrnr$train(DFull_task))
  DFullReg <- DFull_fit_sl$predict(DFull_data_pred_task)
  DFullReg_R2 <- 1 - sum((DFull-DFullReg)^2)/sum((DFull-mean(DFull))^2)
  ...
  fn <- function(args) {
    ...
    pi_gamma <- plogis(qlogis(res.twoStage$pi)+gamma*DFullNCReg/res.twoStage$pi)
    psi <- mean(Delta.W/pi_gamma*(Q1W_eps-Q0W_eps))
    DFullReg_delta <- DFullReg+delta*Delta.W/pi_gamma
    DFull_eps <- HAW*(Y-QAW_eps)+Q1W_eps-Q0W_eps-psi
    score_1 <- mean(Delta.W/pi_gamma*(DFull_eps))
    score_2 <- mean()
