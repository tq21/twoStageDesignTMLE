...
imp_plugin_oracle_Q_g <- function(Y, A, W, W_all, Delta.W, W.stage2, QAW, Q0W, Q1W, g1W, pool=TRUE,
  ...
  # using the oracle QAW, Q0W, Q1W, g1W
  ...
  if (pool) {
    DFullNCReg <- estimateDFullReg_pool(DFull = DFullNC,
                                        ...
                                        discreteSL = TRUE,
                                        Vfold = argList$V.Q)
    # DFullNCReg <- estimateDFullReg_pool_hal(DFull = DFullNC,
    #                                         Delta = Delta.W,
    #                                         V = res.twoStage$d.pi)
  } else {
    DFullNCReg <- estimateDfullReg(DFull = DFullNC[Delta.W == 1],
                                   Delta = Delta.W,
                                   V = res.twoStage$d.pi,
                                   DFullbounds = c(-Inf, Inf),
                                   DFullform = NULL,
  ...
  if (pool) {
    GAWReg <- estimateDFullReg_pool(DFull = GAW,
                                    ...
                                    discreteSL = TRUE,
                                    Vfold = argList$V.Q)
    # GAWReg <- estimateDFullReg_pool_hal(DFull = GAW,
    #                                     Delta = Delta.W,
    #                                     V = res.twoStage$d.pi)
  } else {
    GAWReg <- estimateDfullReg(DFull = GAW[Delta.W == 1],
                               Delta = Delta.W,
                               V = res.twoStage$d.pi,
                               DFullbounds = c(-Inf, Inf),
                               DFullform = NULL,
