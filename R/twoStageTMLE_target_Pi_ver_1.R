...
#' @export
twoStageTMLE_target_Pi_ver_1 <- function(Y, A, W, Delta.W, W.stage2, Z=NULL,
  ...
  H <- DFullReg/res.twoStage$pi
  logit_pi <- qlogis(res.twoStage$pi)
  rootfun <- function(eps) {
    pi_star <- plogis(logit_pi + eps * H)
    mean(H*(Delta.W-pi_star))
  }
  eps_star <- uniroot(rootfun, c(-20, 20))$root
  res.twoStage$pi <- plogis(logit_pi+eps_star*H)
